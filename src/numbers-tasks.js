...
function getFibonacciNumber(index) {
  ...
  for (let i = 3; i <= index; i += 1) {
    const currentFib = prevPrevFib + prevFib;
    prevPrevFib = prevFib;
    prevFib = currentFib;
  }
  return index === 0 ? 0 : prevFib;
}
...
function getSumOfNumbers(x1, x2, x3) {
  if (Math.abs(x1) < 1 || Math.abs(x2) < 1 || Math.abs(x3) < 1) {
